...
//const int length_chac=300;//特征向量维数
//const int length_ori=1200;
...
//    vector<vector<vector<double>>> ghost_test(test_num,vector<vector<double>>(per_num_test_max,vector<double>(length_chac)));
...
//	string address;
//
//	vector<double> all_m_pca(length_chac);
//
//	CvMat* pData = cvCreateMat( total_train+total_test, length_ori, CV_64FC1 );
//    CvMat* pMean = cvCreateMat(1, length_ori, CV_64FC1);
//    CvMat* pEigVals = cvCreateMat(1, min(total_train+total_test,length_ori), CV_64FC1);
//    CvMat* pEigVecs = cvCreateMat( min(length_ori,total_train+total_test), length_ori, CV_64FC1);
//    CvMat* pResult = cvCreateMat( total_train+total_test, length_chac, CV_64FC1 );
//
...
//			   {
//				   data=*(mem+mm);
//				   cvmSet(pData,j+num_individual_sum,mm,data);
//			   }
//			}
...
//			   }
//			}
//		    num_individual_sum+=per_num_test[i];
//	}
...
//		{
//		     data=(double)CV_MAT_ELEM(*pResult,double,j+num_individual_sum,mm);
//			 all_m_pca[mm]+=data;
//	    }
//  }
//	 num_individual_sum+=person_num_individual[i];
...
//			 {
//				 data=(double)CV_MAT_ELEM(*pResult,double,j+num_individual_sum,k);
//				 ghost[i][j][k]=data-all_m_pca[k];
//			 }
//			}
//	    num_individual_sum+=person_num_individual[i];
//	}
...
//			 {
//				 data=(double)CV_MAT_ELEM(*pResult,double,j+num_individual_sum,k);
//				 ghost_test[i][j][k]=data-all_m_pca[k];
//			 }
//		}
//		   num_individual_sum+=per_num_test[i];
//	  }
...
// 
//	 //求F 文档2公式（5）
//	 ni1=cvInvert(Se_mat, F, CV_LU);//CV_LU -最佳主元选取的高斯消除法; CV_SVD - 奇异值分解法 (SVD); CV_SVD_SYM - 正定对称矩阵的 SVD 方法
//     cout<<ni1<<endl;
//	 //invert指令验证
//	 //求G 文档2公式（6）
//	 cvmMul(Su_mat,F,G2);
